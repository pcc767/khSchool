				...
				for(int i = 0; i < nList.getLength(); i++) {
					...
					if(node.getNodeType() == Node.ELEMENT_NODE) {
						try {
							...
							long salesamt = getLogData(eElement, "salesAmt");
							double salesshare = getDoubleData(eElement, "salesShare");
							long salesinten = getLogData(eElement, "salesInten");
							double saleschange = getDoubleData(eElement, "salesChange");
							long salesacc = getLogData(eElement, "salesAcc");
							long audicnt = getLogData(eElement, "audiCnt");
							long audiinten = getLogData(eElement, "audiInten");
							double audichange = getDoubleData(eElement, "audiChange");
							long audiacc = getLogData(eElement, "audiAcc");
							long scrncnt = getLogData(eElement, "scrnCnt");
							long showcnt = getLogData(eElement, "showCnt");
							Boxoffice box = new Boxoffice(0, rnum, rank, rankInten, rankOldAndNew, moviecd, movienm, opendt, salesamt, salesshare, salesinten, saleschange, salesacc, audicnt, audiinten, audichange, audiacc, scrncnt, showcnt, boxofficeType, showRange, yearWeekTime); 
							list.add(box);
						} catch (Exception e){
